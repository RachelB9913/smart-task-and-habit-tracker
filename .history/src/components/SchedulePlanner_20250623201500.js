...
function autoPlaceHabits(habits, currentSchedule, tasks) {
  ...
  habits.forEach(habit => {
    if (!habit.frequency || !habit.preferredTime) {
      console.warn(`Skipping habit ${habit.title} — missing frequency or preferredTime`);
      return;
    }

    const candidateDays = getDaysFromFrequency(habit.frequency); // all valid days
    const candidateHours = getPreferredHours(habit.preferredTime); // all preferred hours

    candidateDays.forEach(day => {
      candidateHours.forEach(hour => {
        const slotKey = `${day}-${hour}:00`;

        const isSlotTaken = (slotKey in newSchedule) && (
          newSchedule[slotKey].includes(`habit-${habit.id}`) ||
          newSchedule[slotKey].some(id => id.startsWith(`habit-${habit.id}-copy`))
        );
        ...
        if (!isSlotTaken) {
          ...
          console.log(`Placed ${habit.title} in ${slotKey}`);
        }
      });
    });
  });
...
export default function SchedulePlanner() {
  ...
  const handleAutoPlace = () => {
    console.log("✨ Auto-placing habits...");
