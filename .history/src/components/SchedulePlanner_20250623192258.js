...
export default function SchedulePlanner() {
  ...
  useEffect(() => {
    const savedClones = localStorage.getItem("habitClones");
    if (savedClones) {
      setHabitClones(JSON.parse(savedClones));
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("habitClones", JSON.stringify(habitClones));
  }, [habitClones]);
                      ...
                      <Droppable droppableId={slotId} key={slotId}>
                        {(provided, snapshot) => (
                          ...
                          >
                            {items.map((id, i) => {
                                let habit;
                                if (id.startsWith("habit-clone-")) {
                                  habit = habitClones.find(clone => clone.id === id);
                                } else if (id.startsWith("habit-")) {
                                  const habitId = id.replace("habit-", "");
                                      ...
                                      >
                                        ...
                                        <div className="habit-icons">
                                          <button
                                            ...
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              const newId = `habit-clone-${habit.id}-${Date.now()}`;
                                              const newClone = {
                                                id: newId,
                                                title: habit.title,
                                                originalHabitId: habit.id
                                              };
                                              setHabitClones((prev) => [...prev, newClone]);

                                              // Place near current slot
                                              ...
                                              for (let offset = -1; offset <= 1; offset++) {
                                                ...
                                                if (h >= 6 && h <= 23) nearbySlots.push(`${currentDay}-${h}:00`);
                                              }
                                              ...
                                              const targetSlot = nearbySlots.find(slot => {
                                                ...
                                                return !items.includes(newId); // prevent same clone twice
                                              }) || `${currentDay}-${Math.min(currentHour + 1, 23)}:00`;

                                              setScheduledTasks(prev => {
                                                const updated = { ...prev };
                                                if (!updated[targetSlot]) updated[targetSlot] = [];
                                                updated[targetSlot].push(newId);
                                                return updated;
                                              });
                                            }}
                                          >➕</button>
