...
export default function SchedulePlanner() {
  ...
  const onDragEnd = (result) => {
    ...
    setScheduledTasks((prev) => {
      ...
      const isHabit = draggableId.startsWith("habit-");

      if (!isHabit) {
        // For tasks: remove from all slots first
      ...
      // Prevent placing the same habit more than once in the same slot
      if (isHabit && currentValues.includes(draggableId)) {
        return updated; // Skip adding duplicate habit to the same slot
      }

      // For habits: allow multiple placements across slots
      // For tasks: only allow if not already placed
      if (isHabit || !Object.values(updated).flat().includes(draggableId)) {
