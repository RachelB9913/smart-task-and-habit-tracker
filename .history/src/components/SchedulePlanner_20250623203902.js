                            ...
                            {items.map((id, i) => {
                              if (id.startsWith("habit-")) {
                                ...
                                if (id.includes("-clone-")) {
                                  const clone = habitClones.find(c => c.id === id);
                                        ...
                                        <div className="habit-icons">
                                          <button
                                            ...
                                            onClick={(e) => {
                                              ...
                                              const cloneId = `habit-${habit.id}-clone-${Date.now()}-${Math.random().toString(36).slice(2)}`;
                                              const newClone = { id: cloneId, habitId: habit.id };
                                              ...
                                              const targetSlot = nearbySlots.find(slot => {
                                                ...
                                                return !items.some(item =>
                                                  item === `habit-${habit.id}` || item.startsWith(`habit-${habit.id}-clone`)
                                                );
                                              });

                                              const fallbackSlot = `${currentDay}-${Math.min(currentHour + 1, 23)}:00`;
                                              const slotToUse = targetSlot || fallbackSlot;

                                              setScheduledTasks((prev) => {
                                                const updated = { ...prev };
                                                if (!updated[slotToUse]) updated[slotToUse] = [];
                                                updated[slotToUse].push(cloneId);
                                                return updated;
                                              });
                                            }}
                                          >➕</button>
