...
export default function Dashboard() {
  ...
  function getPreferredHours(preferredTime) {
    if (preferredTime.includes(':')) {
      return [parseInt(preferredTime.split(':')[0])];  // e.g., '14:00' → [14]
    }
    return timeBlocks[preferredTime.toLowerCase()] || [];
  }

  function getDaysFromFrequency(freq) {
    if (freq === 'Daily') return days;
    if (freq.includes(',')) return freq.split(','); // e.g., ['Mon', 'Wed']
    if (freq.startsWith('Every')) {
      const step = parseInt(freq.match(/\d+/)[0]);
  ...
  return (
    <div className={`dashboard-container ${background}`}>
      ...
      <main>
        ...
        <button
          onClick={() => navigate("/schedule", { state: { tasks } })}
