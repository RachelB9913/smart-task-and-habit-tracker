  ...
  useEffect(() => {
   ...
   const onDragEnd = (result) => {
    ...
    setScheduledTasks((prev) => {
      ...
      const isArray = Array.isArray(slotItems);
      const currentValues = isArray ? slotItems : [slotItems];

      if (draggableId.startsWith("habit-")) {
        // Allow multiple placements
        updated[destination.droppableId] = [...currentValues, draggableId];
      } else {
        // Only allow task placement if it hasn't been scheduled yet
        const alreadyScheduled = Object.values(updated).flat().includes(draggableId);
        if (!alreadyScheduled) {
          updated[destination.droppableId] = [...currentValues, draggableId];
