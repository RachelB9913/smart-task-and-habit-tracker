...
export default function StatisticsPanel() {
  const [habitStats, setHabitStats] = useState({ week: 0, month: 0, total: 0 });
  const [taskStats, setTaskStats] = useState({ week: 0, month: 0, total: 0 });
  ...
  useEffect(() => {
    ...
    const taskMonth = taskCompletions.filter(entry => isSameMonth(entry.completedAt, now)).length;
    const taskTotal = taskCompletions.length;
    setTaskStats({ week: taskWeek, month: taskMonth, total: taskTotal });

    // ---- HABITS ----
    const habitCompletions = JSON.parse(localStorage.getItem("habitCompletions") || "[]");
    const habitWeek = habitCompletions.filter(entry => isSameWeek(entry.completedAt, now)).length;
    const habitMonth = habitCompletions.filter(entry => isSameMonth(entry.completedAt, now)).length;
    const habitTotal = habitCompletions.length;
    setHabitStats({ week: habitWeek, month: habitMonth, total: habitTotal });
